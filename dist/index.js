        ...
        try {
            ...
            app.post('/register', (req, res) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    yield db.collection('Users').insertOne({ username, password: hashedPassword });
            ...
            app.post('/login', (req, res) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const token = jsonwebtoken_1.default.sign({ userId: user._id }, JWT_SECRET, { expiresIn: '1h', algorithm: 'HS256' });
                    res.json({ token });
                }
                catch (error) {
            ...
            app.get('/tasks', authenticateToken, (req, res) => __awaiter(this, void 0, void 0, function* () {
                try {
                    const tasks = yield db.collection('Tasks').find({ userId: req.user.userId }).sort({ order: 1 }).toArray();
                    res.json(tasks);
                }
                catch (error) {
            ...
            app.post('/tasks', authenticateToken, (req, res) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    const highestOrderTask = yield db.collection('Tasks').findOne({ userId: req.user.userId }, { sort: { order: -1 } });
                    const newOrder = highestOrderTask ? highestOrderTask.order + 1 : 0;
                    const result = yield db.collection('Tasks').insertOne({ text: taskText, createdAt, order: newOrder, userId: req.user.userId });
            ...
            app.put('/tasks/:taskId', authenticateToken, (req, res) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    yield db.collection('Tasks').updateOne({ _id: new mongodb_1.ObjectId(taskId), userId: req.user.userId }, { $set: { text: text } });
                    const tasks = yield db.collection('Tasks').find({ userId: req.user.userId }).sort({ order: 1 }).toArray();
            ...
            app.delete('/tasks/:taskId', authenticateToken, (req, res) => __awaiter(this, void 0, void 0, function* () {
                const { taskId } = req.params;
                try {
                    const result = yield db.collection('Tasks').deleteOne({ _id: new mongodb_1.ObjectId(taskId), userId: req.user.userId });
            ...
            app.get('/completedTasks', authenticateToken, (req, res) => __awaiter(this, void 0, void 0, function* () {
                try {
                    const completedTasks = yield db.collection('CompletedTasks').find({ userId: req.user.userId }).toArray();
            ...
            app.post('/tasks/:taskId/complete', authenticateToken, (req, res) => __awaiter(this, void 0, void 0, function* () {
                const { taskId } = req.params;
                try {
                    const task = yield db.collection('Tasks').findOne({ _id: new mongodb_1.ObjectId(taskId), userId: req.user.userId });
                    if (!task) {
                        return res.status(404).json({ error: "Task not found" });
                    }
                    yield db.collection('CompletedTasks').insertOne(Object.assign(Object.assign({}, task), { completedAt: new Date().toISOString() }));
                    yield db.collection('Tasks').deleteOne({ _id: new mongodb_1.ObjectId(taskId), userId: req.user.userId });
            ...
            app.delete('/completedTasks/:completedTaskId', authenticateToken, (req, res) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    const result = yield db.collection('CompletedTasks').deleteOne({ _id: new mongodb_1.ObjectId(completedTaskId), userId: req.user.userId });
                    if (result.deletedCount === 0) {
